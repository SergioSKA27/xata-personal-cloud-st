...
def upload_file(file, description: str = ""):  # This Uploads a file to Xata
    ...
    try:
        if "text" not in file.type:
            xata.upload_file(
                "files", data["id"], "content", file.read(), content_type=file.type
            )  # Upload file
...
def render_file(file: dict):
    try:
        if "text_file" in file and file["text_file"] is not None:
            ...
            file["content"]["mediaType"] == "image/png"
            or file["content"]["mediaType"] == "image/jpg"
            or file["content"]["mediaType"] == "image/jpeg"
            or file["content"]["mediaType"] == "image/gif"
            or file["content"]["mediaType"] == "image/bmp"
            or file["content"]["mediaType"] == "image/svg+xml"
            or file["content"]["mediaType"] == "image/webp"
        ):
            st.write(file["content"]["name"])
            render_img(file)
        elif (
            file["content"]["mediaType"] == "video/mp4"
            or file["content"]["mediaType"] == "video/webm"
            or file["content"]["mediaType"] == "video/ogg"
        ):
            st.write(file["content"]["name"])
            render_video(file)
        elif (
            file["content"]["mediaType"] == "audio/mp3"
            or file["content"]["mediaType"] == "audio/wav"
            or file["content"]["mediaType"] == "audio/ogg"
            or "audio/mpeg" in file["content"]["mediaType"]
        ):
            st.write(file["content"]["name"])
            render_audio(file)
        elif (
            file["content"]["mediaType"] == "application/vnd.ms-excel"
            or file["content"]["mediaType"]
...
def app():
    ...
    if "files" not in st.session_state or st.session_state.files is None:
        st.session_state.files = [
            xata.query(
                "files", {"page": {"size": 6}, "sort": {"xata.createdAt": "desc"}}
            )
        ]
    ...
    if colsp[2].button("⏭️", use_container_width=True):
        if len(st.session_state.files) > st.session_state.page + 1:
            ...
            st.session_state.files.append(
                xata.next_page(
                    "files", st.session_state.files[st.session_state.page], pagesize=6
                )
            )
            st.session_state.page += 1
